...
class Monitor:
    ...
    def laser_low_cb(self, scan):
        ...
        for i in range(len(scan.ranges)):
            ...
            if (-self.platform_width) / 2.0 < dy < self.platform_width / 2.0:
                self.zone1 = True
                if self.dx1 > dx:
                    self.dx1 = dx
            elif self.platform_width / 2.0 < dy < self.platform_width / 2.0 + self.arm_len:
                self.zone2_low = True
                if self.dx2_low > dx:
                    self.dx2_low = dx
    ...
    def command_cb(self, cmd):
        ...
        if self.dx1 > State.SAFE and dx2 > State.SAFE:
            self.state=State.SAFE
        if self.zone1 and State.SAFE < self.dx1 < State.WARNING or self.zone2_high and self.zone2_low and State.SAFE < dx2 < State.WARNING:
            self.state = State.WARNING
        if State.COLLISION < self.dx1 < State.WARNING or  self.zone2_high and self.zone2_low and State.COLLISION < dx2 < State.WARNING:
